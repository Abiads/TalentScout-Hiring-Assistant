...
# Hardcoded API Key
GROQ_API_KEY = "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"
...
def main():
    ...
    try:
        ...
        # Use LCEL Chain instead of deprecated ConversationChain
        # We pass history manually if needed, but for question generation we use stateless calls
        prompt_template = get_persona_prompt(selected_persona)
        conversation = prompt_template | conversation_llm
    except Exception as e:
    ...
    if not st.session_state.candidate_info:
        ...
        # Privacy Notice
        st.info("""
        **Privacy Notice:** Your personal information will be used solely for this technical assessment. 
        ...
        For complete privacy policy, see [PRIVACY.md](https://github.com/Abs6187/TalentScout-AI/blob/main/PRIVACY.md).
        """)
        ...
        with st.form('info_form'):
            ...
            # Hidden processing for submission consistency
        ...
        st.markdown(r"*Required fields are marked with an asterisk (\*)")
        ...
        with col1:
            if st.button('Submit Answer'):
                if not answer.strip():
                    ...
                    with col_sent2:
                        st.metric("Response Confidence", f"{sentiment_data['confidence_score']*100:.0f}%")
                    with col_sent3:
                        st.metric("Sentiment", f"{sentiment_data['sentiment']}")
                    ...
                    with st.expander("View Detailed Feedback"):
                        ...
                        st.write(f"- Word Count: {sentiment_data['word_count']}")
                        st.write(f"- Technical Depth Indicators: {sentiment_data['technical_depth']}")
                        st.write(f"- Confidence Indicators: {sentiment_data['positive_indicators']}")
                        st.write(f"- Uncertainty Indicators: {sentiment_data['uncertain_indicators']}")
