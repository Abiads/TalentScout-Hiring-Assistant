...
Remember: Return only valid JSON, no other text."""
    ...
    try:
        # Get LLM response using invoke() instead of deprecated predict()
        response = evaluation_llm.invoke(prompt).content
...
def generate_detailed_feedback_with_llm(answers, tech_stack):
    ...
    try:
        detailed_feedback = feedback_llm.invoke(prompt).content
...
"""
    ...
    try:
        recommendation = recommendation_llm.invoke(prompt).content
